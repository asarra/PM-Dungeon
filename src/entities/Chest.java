...
/**
 * Klasse zur Darstellung einer Schatztzuhe im Spiel Scgatztuhen spawnen in zufälligen Positionen im
 * Dungeon
 */
public class Chest extends GameActor {
  ...
  private final int XPREWARD = 20;
  private boolean isChestOpen = false;
  private boolean logged = false;
  private boolean isXPgiven = false;
  ...
  public Chest() {
    ...
    for (; i <= INVENTORYCAPACITY; i++) {
      ...
      for (j = 0; j < jMax; j++) this.addToInventory(this.itemFactory.getWeapon(Weapons.SWORD));
      summe += j + k + l;
      if (summe == 3) {
        break;
      }
      ...
      for (k = 0; k < kMax; k++) this.addToInventory(this.itemFactory.getWeapon(Weapons.AXE));
      summe += j + k + l;
      if (summe == 3) {
        break;
      }
      ...
      for (l = 0; l < lMax; l++) this.addToInventory(this.itemFactory.getPotion());
      summe += j + k + l;
      if (summe == 3) {
        break;
      }
  ...
  public void open(Hero hero) {
    ...
    if (!logged) {
      LOGGER.log(
          Level.INFO,
          "Folgende Items sind in der Schatzkiste: " + this.inventory.returnItemNames());
      if (!inventory.isEmpty()) {
        ...
        if (!isXPgiven) {
          hero.addExpPoints(XPREWARD);
          hero.notifyQuests();
          isXPgiven = true;
          LOGGER.info("YOU GOT " + XPREWARD + "XP FROM OPENING THIS CHEST");
        }
      }
      logged = true;
    }
