...
/** Abstrakte Klasse zur Darstellung eines Gegners im Spiel */
public abstract class Enemy extends GameActor {
  ...
  public void update() {
    ...
    if (this.targetPoint == null) this.resetTargetPoint();
    ...
    // Wenn der übergebene Punkt nicht betretbar ist oder die Zielkoordinate und die jetzige
    // Koordinate gleich sind, wird die Zielkoordinate erneuert
    // Musste es umschreiben, da sonst der untere else if die Checks im jetzigen if blockiert. (Saß
    // Stunden daran, um herauszufinden, dass es daran lag)
    if (!level.isTileAccessible(newPosition)
        | (Math.round(this.position.x) == this.targetPoint.x
            & Math.round(this.position.y) == this.targetPoint.y)) {
